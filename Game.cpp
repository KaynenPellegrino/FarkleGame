// IT 312- Farkle.cpp : Defines the entry point for the application.
// This file will make up most of the game including player creation, dice, turns, scores, and determining the winner.
...
//clears the input when invalid characters are used
void Game::ClearInput()
{
	//if an invalid number is detected it will clear
	cin.clear();
	
	cin.ignore(numeric_limits<streamsize>::max(). '\n');
}

//returns an integer for the player input
int Game::GetInput()
{
	unsigned int sel;
...
void Game::Congratulate(Player& player)
{
	system("cls");
	cout << "******************************" << endl;
	cout << "YOU DID IT! YOU'RE THE WINNER!" << endl;
	cout << "******************************" << endl;
	cout << player.Name() << " is the winner with a final score of " << player.Score() << ". Brag as you wish." << endl << endl;

	system("pause");
}
//inform of final round

void Game::FinalRound(Player& player)
{
	system("cls");
	cout << "********************************" << endl;
	cout << "YOU HAVE ENTERED THE FINAL ROUND" << endl;
	cout << "********************************" << endl;
	cout << player.Name() << " has a score of " << player.Score() << ". Well Played." << endl;
	cout << "The final round of play has finally begun! May the odds be ever in your favor." << endl << endl;
